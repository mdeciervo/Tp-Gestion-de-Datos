...
namespace FrbaHotel.ABM_de_Rol
{
    public partial class Baja : Form
    {
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 0)
            {
                string query = "select F.idFuncionalidad ID,F.denominacion Denominacion from GESTION_DE_GATOS.Funcionalidad F, GESTION_DE_GATOS.FuncXRol FR where F.idFuncionalidad = FR.funcionalidad and FR.rol = " + dataGridView1.CurrentRow.Cells[2].Value.ToString(); ;
            ...
            if (e.ColumnIndex == 1)
            {
                ...
                {
                    string consulta = "update GESTION_DE_GATOS.Rol set estado=0 where idRol = " + id;
